<template>
    <AppLayout title="Danh Sách Thể Loại">
        <template #header>
            ...
            <div class="max-w-12xl mx-auto sm:px-6 lg:px-8">
                <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg pt-8 pb-12 px-2">
                    ...
                    <div class="float-right text-xs uppercase">
                        <Link :href="route('Type.Create')" class="bg-green-600 hover:bg-green-700 text-white font-bold py-2 px-2 rounded ">
                        <icon :icon="['fas', 'plus']" /> Thêm dữ liệu
                        </Link>
                    </div>

                    <div class="my-2 py-10">
                        <DataTable :loading="loading" :headers="headers" :items="dataPage" buttons-pagiType show-index v-model:items-selected="itemsSelected">
                            ...
                            <template #item-operation="{ id, name }">
                                <div class="py-3 flex items-center justify-center">
                                    ...
                                    <Link :href="route('Type.Edit', id)" class="bg-yellow-600 text-white px-2 py-1 rounded-md mr-5">
...
export default {
    ...
    setup() {
        ...
        const loadData = () => {
            loading.value = true;
            axios.post('type/load-data-table')
                .then((response) => {
        ...
        return {
            dataPage, loadData, searchDataTable, searchName, loading
        }
    },
