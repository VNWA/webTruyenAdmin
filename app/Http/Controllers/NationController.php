...
class NationController extends Controller
{
    ...
    function showIndex()
    {
        ...
        return Inertia::render('Nation/Show', ['data' => $data]);
    }
    ...
    function create(Request $rq)
    {
        ...
        if (!$rq->slug) {
            ...
            if (Nation::where('slug', $rq->slug)->first()) {
    ...
    function showEdit($id)
    {
        ...
        return Inertia::render('Nation/Edit', ['data' => $data]);
    }
    ...
    function update(Request $rq, $id)
    {
        ...
        if (!$rq->name) {
            return response()->json(['error' => 'Vui lòng nhập tên quốc gia', 'column' => 'name']);
        } else {
            $data['name'] = $rq->name;
        }
        if (!$rq->slug) {
            return response()->json(['error' => 'Có lỗi xảy ra, vui lòng load lại trang', 'column' => 'slug']);
        } else {
            if (
                Nation::where('slug', $rq->slug)
                    ->whereNotIn('id', [$id])
                    ->first()
            ) {
                return response()->json(['error' => 'Nhập đường dẫn khác', 'column' => 'slug']);
            } else {
                $data['slug'] = $rq->slug;
            }
        }
        ...
        try {
            Nation::where('id', $id)->update($data);
        } catch (\Throwable $th) {
